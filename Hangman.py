...
words = ['lemon','melon','bingo','chalk','peach','mango']

#words = ['apple','banana','orange','coconut','strawberry','lime','grapefruit','lemon','kumquat','blueberry','melon']

while True:
    #Step 3
    start = input("Press enter S return to start, or enter Q to quit\n")
    ...
    while len(bad_guesses) < 7 and len(good_guesses) != len (list( secret_word)):
        ...
        if len (good_guesses) == len ( list(secret_word)):
            print ("Hurray!!!You win : The word was {}". format(secret_word))
            break
        else:
